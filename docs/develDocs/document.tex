...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
  ...
  \node[state]         (E) [below  of=B] {5};
  \node[state]         (F) [below  of=C] {6};
  \node[state]         (G) [below right of=C] {7};

  \node[state]         (H) [below left of=D] {8};
  \node[state]         (I) [below of=H] {9};
  \node[state]         (J) [below of=I] {10};
  \node[state]         (K) [below of=J] {11};
  
  \node[state, fill=black!20]         (L) [right of=J] {12};
  \node[state, fill=black!20]         (M) [right of=L] {13};
  \node[state, fill=black!20]         (N) [right of=M] {14};

  \node[state, fill=black!20]         (O) [right of=K] {15};
  \node[state, fill=black!20]         (P) [right of=O] {16};
  \node[state, fill=black!20]         (Q) [right of=P] {17};
  \node[state, fill=black!20]         (R) [right of=Q] {18};

  \path (A) edge [bend right] node {SES:001} (B)
            edge [bend left]  node {SES:002} (C)
        (B) edge [bend right] node {BNDL:msajc003} (D)
            edge              node {BNDL:msajc010} (E)
        (C) edge              node {BNDL:x} (F)
            edge [bend left]  node {BNDL:y} (G)
        (D) edge [bend right] node {LEV:l1} (H)
            edge [bend right] node {LEV:l2} (I)
            edge [bend right] node {LEV:l3} (J)
            edge [bend right] node {LEV:l4} (K)
        (J) edge              node {START} (L)
        ...
        (K) edge              node {START} (O)
        (O) edge [bend left]  node {NEXT} (P)
        (P) edge [bend left]  node {PREV} (O)
        (P) edge [bend left]  node {NEXT} (Q)
        (Q) edge [bend left]  node {PREV} (P)
