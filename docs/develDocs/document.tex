...
\begin{itemize}
  ...
  \item \texttt{allowedValues} specifies the possible entries that are allowed. For example \texttt{["a","b","c"]} would only allow the labels a, b and c to be used. If undefined or empty $\rightarrow$ no restrictions. Good idea?? Or should it always be set?
  \item \texttt{"type":"txt"} what types are supported? Should we support anything else but \texttt{"txt"}. If no then why specify a field for it?
\end{itemize}
